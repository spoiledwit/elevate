...
class Command(BaseCommand):
    help = 'Create TWC (The Wealth Creator) opt-in follow-up email templates in sequence'

    def add_arguments(self, parser):
        parser.add_argument(
            '--recreate',
            action='store_true',
            help='Delete existing TWC email templates and create new ones',
        )

    def handle(self, *_args, **options):
        if options['recreate']:
            self.stdout.write('Deleting existing TWC email templates...')
            OptinFollowupEmail.objects.filter(program='TWC').delete()

        # Define TWC email templates (plain text with template variables)
        email_templates = [
            {
                'program': 'TWC',
                'step_number': 1,
...
- {{ sender_name }}''',
        ...
        for email_data in email_templates:
            self.stdout.write(f'Creating TWC email {email_data["step_number"]}...')
        ...
        self.stdout.write(f'Total TWC email templates created/updated: {len(created_emails)}')

        # Display email schedule
        self.stdout.write('\nTWC Email Sequence Schedule:')
        ...
        self.stdout.write(self.style.SUCCESS('\n✓ TWC email sequence creation completed!'))
